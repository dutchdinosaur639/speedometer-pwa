...
const parsePosition = (position) => {
  ...
  if (appOpts.speedHistory.length > 10) {
    appOpts.speedHistory.shift();
  }

  appOpts.powerHistory.push(power);
  if (appOpts.powerHistory.length > 10) {
    appOpts.powerHistory.shift();
  }

  const averageSpeed =
    appOpts.speedHistory.reduce((a, b) => a + b, 0) /
    appOpts.speedHistory.length;
  const averagePower =
    appOpts.powerHistory.reduce((a, b) => a + b, 0) /
    appOpts.powerHistory.length;
